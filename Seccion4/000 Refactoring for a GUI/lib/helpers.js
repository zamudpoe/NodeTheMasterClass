...
// Obtenemos el contenido en string de una plantilla en el directorio de plantillas "./templates/"
helpers.getTemplate = function (templateName, callback){
  templateName = typeof(templateName) == 'string' && templateName.length > 0 ? templateName : false

  if (templateName) {
    var templatesDir = path.join(__dirname,'../templates/')
    fs.readFile(templatesDir+templateName+'.html', 'utf8', function (err, data) {
      (!err && data && data.length > 0) ? callback(false, data) : callback ('No template could be found')
    })
  } else {
